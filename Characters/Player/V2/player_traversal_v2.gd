extends Node
## Shared ownership lifecycle; Mantle provides the Phase 1B physical profile.
...
func advance(delta: float) -> void:
	...
	if mantle.running: return # Actual evaluated animation owns the mantle clock.
...
func request_traversal_interrupt(reason: int) -> bool:
	if mantle.running:
		if phase!=Phase.ENTRY or not traversal_interruptible or not traversal_interrupt_window_open or reason not in [Interrupt.DODGE,Interrupt.JUMP,Interrupt.DAMAGE,Interrupt.PLAYER_CANCEL]: return false
