...
The existing geometry query walks the local ledge tangent in .025m samples, retaining curved normals and all brace, clearance, height and turn checks. Full distance is tested first. On failure, its first invalid sample bounds the search; a backward coarse search (at most 12 steps) finds a valid bracket and 8 refinements locate its boundary. Geometry search is followed by the full 240-segment authored-arc capsule sweep. If that sweep fails, the bounded search repeats with arc validation per candidate. No destination-only acceptance and no sharp-corner bypass. The final request rechecks the full committed arc; ordinary per-tick collision validation remains active.

`actual_distance = min(desired_distance, available_safe_hop_distance)`, accepted only at/above the minimum. Available here means a complete-animation-safe final displacement, not raw distance to the mesh edge. Existing bilateral hand room and authored overshoot must also fit. Therefore a .75m geometric remainder produces about .724m left / .730m right; .38m produces about .367m / .370m; .05m produces no hop. Desired full 3m remains 3m when sufficient space exists. This reserve prevents the clip from moving beyond the safe edge before settling back.

The normalized authored lateral curve is multiplied by actual distance and evaluated on the existing piecewise curved route. Full clip duration, acceleration/deceleration shape, and overshoot shape are retained. **Vertical arc and normal retreat are unchanged**; no additional vertical scaling was introduced. Destination alignment, facing, hand targets and foot rebrace use the selected route and finish at its exact final anchor. The existing .12s hop-to-idle blend, shared body offsets and hand/foot correction limits are untouched. No camera edits.

## Priority, previews and diagnostics

Resolution is full continuous hop → useful partial continuous hop → existing horizontal gap query → none. A tested .38m remainder takes continuous priority; a .05m remainder allows gap transfer. The transfer regression now consumes a useful continuous remainder before testing its remote jump rather than expecting the former all-or-nothing fallback.

Idle hop previews are cached for at most one second per side/unchanged source transform, anchor and distance settings. A changed anchor or setting invalidates the hint; an obstacle change may take up to one second to appear in the HUD. **Requests never use the cache.** This limits repeated expensive idle searches without making stale previews authoritative. Shimmy previews and shimmy movement are unchanged.

Lateral debug text shows desired, available, minimum, actual and FULL_HOP / PARTIAL_HOP / GAP_TRANSFER / NONE, plus the limiting reason. Markers: white current/start anchor; magenta nominal full-distance tangent projection (not a claimed curved destination); cyan maximum validated target/route; green selected target; yellow minimum threshold; red rejected route beyond the safe portion. The live authored motion path remains orange.

## Playground and verification

New `test/traversal_playground/hang_partial_course.tscn` and generator are instanced as `Hang/PartialHopCourse` at local (165,0,-110), beyond the outward-transfer row, with a connecting floor. Nine labeled fixtures: full, 75%, 50%, 25%, below minimum, corner/end, partial-before-gap, gap-priority, and gentle cylinder curve. Yellow stripes mark starting positions; rear ramps provide access to the straight fixtures. The curved fixture runs into the existing 35-degree per-action turn budget, producing a shortened curved route rather than permitting a sharp turn.

New tests `test_hang_partial_hop_v2.gd` and `test_hang_partial_course_v2.gd` cover both directions, full/useful/tiny distances, complete playback, exact endpoints, no repeated edge drift, preserved vertical arc/arm lengths, curve limit, body protrusion, overhead arc block, no corner rotation, and both gap-priority outcomes. Observed curved partial ~1.784m, body-obstruction partial ~.437m. Existing lateral tuning, gap transfer, acquisition, vertical, top-down camera, hang camera, crouch continuity, normal jump recatch and optional outward-toggle checks were also run. Rendered partial-hop/debug samples inspected; subjective feel of the smallest useful hop remains a play-test tuning point. No source GLB, acquisition implementation, grounded controller, camera or climb code changed.
